...
vignette: >
  %\VignetteIndexEntry{Chapter 06: Deviance residuals, model statistics and posterior inference (+ bayestestR)}
...
## Posterior predictive checks with 'bayesplot' (optional)

See **`legacy_code/pp_check.glmb.R`** and **`legacy_code/README.md`** (method not part of the installed package). **Chapter 12** describes optional **`bayesplot`** workflows when that package is installed separately from CRAN.

<!--
bayesplot not a glmbayes dependency; see legacy_code/pp_check.glmb.R.
Uncomment and install bayesplot from CRAN to run:

    suppressPackageStartupMessages(library(bayesplot))
    bayesplot::pp_check(glmb.out1, ndraws = 5L, fun = "hist")
-->

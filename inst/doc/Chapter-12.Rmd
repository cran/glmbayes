...
## **3. Install glmbayes from source for OpenCL**

The release on CRAN and prebuilt binaries on R‑Universe are built without OpenCL GPU support.
Install from source (from GitHub or R‑Universe) on a machine with OpenCL headers and a working vendor runtime to enable GPU acceleration.
The CRAN release provides the stable, CPU-only build; development source installs follow the same workflow below.
